...
lifecycleLayer("CodexAdapterLive lifecycle", (it) => {
  ...
  it.effect("marks completed child messages with their receive time signal", () =>
    Effect.gen(function* () {
      const { adapter, runtime } = yield* startLifecycleRuntime();
      const firstEventFiber = yield* Stream.runHead(adapter.streamEvents).pipe(Effect.forkChild);

      yield* runtime.emit({
        id: asEventId("evt-child-message"),
        kind: "notification",
        provider: ProviderDriverKind.make("codex"),
        createdAt: "2026-01-01T00:00:00.000Z",
        ...
        payload: {
          ...
          nickname: "researcher",
          itemPhase: "completed",
          item: { type: "agentMessage", id: "msg-child-1", text: "Report ready" },
        },
      } satisfies ProviderEvent);
      ...
      NodeAssert.equal(firstEvent.value.payload.taskId, "child-thread-1");
      NodeAssert.equal(firstEvent.value.payload.messageReceived, true);
    }),
  );
