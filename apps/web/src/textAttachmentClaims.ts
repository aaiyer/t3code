...
export class TextAttachmentClaimReconciler {
  ...
  async #reconcile(): Promise<void> {
    ...
    for (const path of this.#desired) {
      ...
      if (await this.#runOperation("claim", path, () => this.#claim(path))) {
        this.#confirmed.add(path);
      } else failed = true;
    }
    for (const path of this.#confirmed) {
      if (this.#desired.has(path)) continue;
      if (await this.#runOperation("release", path, () => this.#release(path))) {
  ...
  async #runOperation(
    ...
    if (started) {
      this.#inFlight.set(key, actual);
      void actual.then((succeeded) => {
        if (!succeeded) return;
        if (kind === "claim") this.#confirmed.add(path);
        else this.#confirmed.delete(path);
        if (!this.#disposed && !this.#paused) this.#enqueueReconcile(true);
      });
    }
    let timeout: ReturnType<typeof setTimeout> | null = null;
    const outcome = await Promise.race([
      actual.then((succeeded) => ({ settled: true as const, succeeded })),
      new Promise<{ readonly settled: false; readonly succeeded: false }>((resolve) => {
        timeout = setTimeout(
          () => resolve({ settled: false, succeeded: false }),
    ...
    return outcome.settled && outcome.succeeded;
  }
...
export async function retryTextAttachmentOperation(
  ...
  for (let attempt = 0; attempt < maxAttempts && !options.signal?.aborted; attempt += 1) {
    if (await operation().catch(() => false)) return true;
...
function registeredTextAttachmentClaimOperations(input: {
  environmentId: EnvironmentId;
  draftOwnerId: string;
  operations: TextAttachmentClaimOperations;
}): { claim: (path: string) => Promise<boolean>; release: (path: string) => Promise<boolean> } {
  return {
    ...
    release: async (path) => {
      ...
      // Persist before issuing the destructive RPC. A refresh after a failed
      // or interrupted request can then resume the release instead of leaking
      // the server-side draft claim.
      persistPendingTextAttachmentReleases([pendingRelease]);
      const released = await input.operations.release(path, input.draftOwnerId);
      if (released) completePendingTextAttachmentRelease(pendingRelease);
      return released;
    },
  };
}
...
export function getTextAttachmentClaimReconciler(input: {
  ...
  const operations = registeredTextAttachmentClaimOperations(input);
  const existing = textAttachmentClaimReconcilerRegistry.get(key);
  if (existing) {
    existing.setOperations(operations);
    return existing;
  }
...
const TEXT_ATTACHMENT_UPLOAD_FENCE_TIMEOUT_MS = 10_000;

async function waitForTextAttachmentUploads(
  pending: ReadonlyArray<Promise<void>>,
  timeoutMs: number = TEXT_ATTACHMENT_UPLOAD_FENCE_TIMEOUT_MS,
): Promise<void> {
...
export async function fenceTextAttachmentUploadOwner(
  ...
  state.fenced = true;
  await waitForTextAttachmentUploads([...state.pending]);
}
...
export async function fenceTextAttachmentUploadEnvironment(
  ...
  await waitForTextAttachmentUploads(waits);
}
